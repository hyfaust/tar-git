...
class DiffEngine:
    ...
    def diff_default(
        ...
        # Plain files: git diff directly in working dir
        if files:
        ...
        if rels:
            r = run_git(["diff"] + rels, cwd=str(self.working_dir))
            if r.stdout.strip():
                print(r.stdout)
                changed = True
    ...
    def diff_version(
        ...
        return self._git_diff_version(version, targets)
    ...
    def diff_suffix_version(
        ...
        if dc and (dc.cmd or dc.script):
            ...
            if not suffix_files:
                ...
                return False
            return self._git_diff_version(version, suffix_files)
    ...
    def _git_diff_version(
        ...
        if files:
            args.append("--")
            args.extend(files)
        r = run_git(args, cwd=str(self.working_dir))
        if r.stdout.strip():
            print(r.stdout)
            return True
    ...
    def _custom_diff(
        ...
        for node in targets:
            ...
            Path(head_file).unlink(missing_ok=True)
        return changed
    ...
    # ── Internal: extract from git ───────────────────────────────────────

    def _extract_from_head(self, node: FileNode) -> Optional[str]:
        """Extract an archive's content from HEAD into a temp packed file."""
        return self._extract_from_version(node, "HEAD")

    def _extract_from_version(self, node: FileNode, version: str) -> Optional[str]:
        """Extract an archive's content from *version* into a temp packed file."""
        extract_dir = self.processor.extract_path(node)
        rel = extract_dir.relative_to(self.working_dir).as_posix()

        r = run_git(
            ["ls-tree", "-r", f"{version}:{rel}"],
        ...
        temp_dir = Path(td) / "content"
        temp_dir.mkdir(parents=True, exist_ok=True)
        self._restore_from_ls_tree(r.stdout, str(temp_dir), str(self.working_dir))
        ...
        pack_cmd = [
            c.replace("{src}", str(temp_dir)).replace("{dst}", str(temp_file))
    ...
    # ── Fallback suffix diff ─────────────────────────────────────────────

    def _fallback_suffix_diff(
        self, scanner: Scanner, suffix: str, filename: Optional[str]
    ) -> bool:
        """Default diff for a suffix type: hash comparison for archives."""
        targets = self._files_of_suffix(scanner, suffix, filename)
        if not targets:
            print_info(f"No .{suffix} files found")
            return False
