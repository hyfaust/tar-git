...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  tgit rebuild                      restore missing archives\n"
            "  tgit daemon --interval 600        timed auto-save\n"
            "  tgit status                       git status\n"
        ),
    )
    parser.add_argument(
        "--show-version",
    ...
    # rebuild
    subs.add_parser("rebuild", help="Restore missing archives from git")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    dispatch = {
        ...
        "diff":      lambda: _cmd_diff(config, git, scanner, processor, awareness,
                                        args),
        "rebuild":   lambda: _cmd_rebuild(config, git, scanner, processor, awareness),
...
def _git_passthrough(args: List[str]) -> int:
    ...
    if not (working_dir / ".git").exists():
        ...
        return 1
    try:
        r = subprocess.run(["git"] + args, cwd=str(working_dir))
        return r.returncode
...
def _copy_templates(working_dir: Path) -> None:
    ...
    src = project_root / Config.CONFIG_FILENAME
    dst = working_dir / Config.CONFIG_FILENAME
    if src.exists() and not dst.exists():
        shutil.copy2(str(src), str(dst))
        print_info(f"Created {Config.CONFIG_FILENAME} from template")
...
def _cmd_init(
    ...
    # Copy template tgit.toml if missing
    _copy_templates(git.working_dir)

    # Reload config after template copy
    config.__init__(git.working_dir)

    # Init git repo in working directory
    if not git.is_repo():
        r = run_git_local(["init"], cwd=str(git.working_dir))
        if r.returncode != 0:
            print_err(f"Git init failed: {r.stderr.strip()}")
            return False
        print_ok("Git repository created")

    # Generate .gitignore from config
    gitignore_path = config.generate_gitignore(git.working_dir)
    print_ok(f"Generated {gitignore_path.name}")
    ...
    if n_tar > 0:
        ...
        print_ok(f"Extracted {n_tar} archive(s)")

    # Generate hashes
    scanner.scan(force=True)  # Re-scan after extraction
    awareness.update_hashes(scanner)

    # Initial commit
    run_git_local(["add", "-A"], cwd=str(git.working_dir))
    r = run_git_local(
        ["commit", "-m", "tgit: initial commit", "--allow-empty"],
        cwd=str(git.working_dir),
    )
    if r.returncode == 0:
...
def _cmd_restore(
    ...
    # Re-scan after restore and repack any missing archives
    scanner.scan(force=True)
    repacked = 0
    for node in scanner.tar_files():
        extract_dir = processor.extract_path(node)
        if extract_dir.exists() and not node.path.exists():
            if processor.pack_single(node):
                repacked += 1
    if repacked:
        print_ok(f"Repacked {repacked} archive(s)")
...
def _cmd_log(git: GitBackend, limit: int, graph: bool, extra_args: List[str]) -> bool:
    ...
    # Build tag map
    tag_map: dict = {}
    tr = run_git(["show-ref", "--tags", "-d"], cwd=str(git.working_dir))
    ...
    head = run_git(["rev-parse", "HEAD"], cwd=str(git.working_dir))
...
def _cmd_rebuild(
    ...
    if not git.is_repo():
        ...
        return False
    scanner.scan(force=True)
    return awareness.rebuild(scanner, processor, git) >= 0
...
def _open_and_watch(
    ...
    try:
        if sys.platform == "win32":
            subprocess.Popen(["cmd", "/c", "start", "", str(fpath)], shell=True)
